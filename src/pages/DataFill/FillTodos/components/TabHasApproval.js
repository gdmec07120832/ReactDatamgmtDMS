...
const ExpandTable = (props) => {
  ...
  return (
    <div className={'pt-5 pb-2 pr-8 shadow-inner shadow-2xl'} style={{ margin: -8 }}>
...
function TabHasApproval() {
    ...
    columns: [
      {
        ...
        render(text) {
          return (
            <div className={'py-1 break-all line-clamp-3'} title={text}>
             {text}
            </div>
          )
        },
  ...
  const { run: getList, loading } = useRequest(
    () => {
      return axios
        .get('/bi-data-reporting/api/user/workbench/queryApprovedInfo', {
          params: {
            page,
            pageSize,
            ...query,
