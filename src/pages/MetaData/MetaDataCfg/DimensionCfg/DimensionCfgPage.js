...
function DimensionCfgPage(props) {
  ...
  const { table, setTable } = useTable({
    rowKey: 'id',
    columns: [
      { dataIndex: 'dimensionName', title: '名称' },
      {
        dataIndex: 'sortValue',
        title: '排序',
        render(text, record) {
          return (
            <InputNumber
              ...
              onBlur={(e) => {
                Number(e.target.value) !== record.sortValue && updateRowOrder(record, e.target.value)
              }}
              onPressEnter={(e) =>
                Number(e.target.value) !== record.sortValue && updateRowOrder(record, e.target.value)
              }
      ...
      },
      {
        ...
        title: '操作',
        render(text, record) {
          const hasTogglePerm = permissionsMap['bi-metadata.DimensionController.startOrStop']
          return (
            <Space>
              ...
              {permissionsMap['bi-metadata.DimensionController.saveOrUpdate'] && (
                <Button size={'small'} type={'link'} onClick={() => updateRow(record)}>
                  编辑
                </Button>
              )}
              {permissionsMap['bi-metadata.DimensionController.delById'] && (
  ...
  const updateRowOrder = (record, order) => {
    axios
      .post('/bi-metadata/api/user/kpiNode/updateDimensionSortValue', [
        {
          id: record.id,
          sortValue: order,
        },
  ...
  const getList = useCallback(() => {
    ...
    axios
      .get('/bi-metadata/api/admin/dimension/list', {
        params: {
          keyword,
          pageSize,
          page: current,
  ...
  const handleToggle = (record) => {
    axios
      .get('/bi-metadata/api/admin/dimension/startOrStop', {
        params: { id: record.id },
      })
      .then(() => {
  ...
  const deleteRow = (record) => {
    axios
      .get('/bi-metadata/api/admin/dimension/delById', {
        params: { id: record.id },
      })
      .then(() => {
  ...
  const RecordModal = (props) => {
    ...
    const submitForm = () => {
      form.validateFields().then((formData) => {
        axios
          .post('/bi-metadata/api/admin/dimension/saveOrUpdate', {
    ...
    return (
      <DraggableModal
        title={modalForm ? '修改维度' : '新增维度'}
        visible={visible}
        onCancel={() => setVisible(false)}
        onOk={submitForm}>
        <Form form={form}>
          <Form.Item label={'维度名称'} name="dimensionName" rules={[{ required: true, message: '此项必填' }]}>
            <Input placeholder={'维度名称'} maxLength={20} />
          </Form.Item>
        </Form>
      </DraggableModal>
    )
